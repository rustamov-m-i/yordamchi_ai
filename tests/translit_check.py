...
def main() -> None:
    ...
    # "yoʻ" = y + oʻ (ў), NOT yo (ё) + tutuq — very common (yoʻq/yoʻl/Yoʻldosh)
    check("yoʻq → йўқ (not ёъқ)", c("yo'q"), "йўқ")
    check("yoʻl → йўл", c("yo'l"), "йўл")
    check("Yoʻldoshev → Йўлдошев", c("Yo'ldoshev"), "Йўлдошев")
    check("yogʻ → ёғ (apostrophe binds g)", c("yog'"), "ёғ")
    ...
    # TLD-based domain detection: case-insensitive (UPPERCASE domains kept)
    check("UPPER domain: CLICK.UZ", pro("CLICK.UZ"), "CLICK.UZ")
    check("mixed domain: Site.RU", pro("Site.RU"), "Site.RU")
    # name with lowercase surname still transliterates (not a domain)
    check("name lc surname: J.komilov", pro("J.komilov"), "Ж.комилов")
    check("name lc surname: S.aliyev", pro("S.aliyev"), "С.алиев")
    # capitalized word whose prefix == keep term must NOT split (closed suffix set)
    check("no-split: Posta (Post+a)", pro("Posta"), "Поста")
    check("no-split: Linkor (Link+or)", pro("Linkor"), "Линкор")
    check("no-split: Targetlangan", pro("Targetlangan"), "Таргетланган")
    check("feedback kept whole", pro("Feedback"), "Feedback")
